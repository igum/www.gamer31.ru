		...
		<li class="cat_post_item-<?php echo $posts_counter; ?> clearfix">
            ...
            <?php if ( $instance['show_title'] ) : ?>
				<?php if ( $instance['date'] ) : ?>
            	    <div class="post_meta"><time datetime="<?php the_time('Y-m-d\TH:i'); ?>"><?php the_time('j F Y'); ?></time></div>
            	  <?php endif; ?>
            ...
            <?php if ( $instance['more_link'] ) : ?>
              <a href="<?php the_permalink() ?>" class="<?php if($instance['more_link_class']!="") {echo $instance['more_link_class'];}else{ ?>link<?php } ?>"><?php if($instance['more_link_text']==""){ ?>Далее..<?php }else{ ?><?php echo $instance['more_link_text']; ?><?php } ?></a>
            <?php endif; ?>
			</div>
			<div class="rating"><?php if(function_exists('the_ratings')) { the_ratings($start_tag = 'div', $custom_id = $post->ID, $display = true); } ?></div>
		</li>
	<?php } ?>
	<?php echo "</ul>\n"; ?>
...
<div style="width:230px; float:left; padding-left:20px;">
  ...
  <label for="<?php echo $this->get_field_id("more_link_text"); ?>">
    <?php _e( 'Link text' ); ?>:
    <input class="widefat" id="<?php echo $this->get_field_id("more_link_text"); ?>" name="<?php echo $this->get_field_name("more_link_text"); ?>" type="text" value="<?php echo esc_attr($instance["more_link_text"]); ?>" /> <span style="font-size:11px; color:#999;"><?php _e( '(default: "Далее..")' ); ?></span>
  </label>
  </p>
  <p>
