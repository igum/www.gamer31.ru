...
class MY_PostWidget extends WP_Widget {
  ...
  /** @see WP_Widget::widget */
						...
						<?php  $temp = $wp_query;
								...
								<li class="clearfix">
									<?php if(has_post_thumbnail()) { ?>
										...
										$image = aq_resize( $img_url, 220, 220, true ); //resize & crop img
										?>
									...
									<?php } ?>
									  <div class="extra-wrap">
									  	<div class="post-meta"><?php _e('By', 'theme1683'); ?> <?php the_author_posts_link() ?> <?php _e('on', 'theme1683'); ?> <time datetime="<?php the_time('Y-m-d\TH:i'); ?>"><?php the_time('F j, Y'); ?></time> <?php _e('in', 'theme1683'); ?> <?php the_category(', ') ?></div>
									  	<h4><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php _e('Permanent Link to', 'theme1683');?> <?php the_title_attribute(); ?>"><?php the_title(); ?></a></h4>
								...
								<?php if($linkurl !=""){?>
									<a href="<?php echo $linkurl; ?>" class="link-2"><?php echo $linktext; ?></a>
								<?php } ?>
    ...
    function form($instance) {				
      ...
      <p><label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Category Slug:', 'theme1683'); ?> <input class="widefat" id="<?php echo $this->get_field_id('category'); ?>" name="<?php echo $this->get_field_name('category'); ?>" type="text" value="<?php echo $category; ?>" /></label></p>
